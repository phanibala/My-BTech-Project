...
function [MSE,PSNR,AD,SC,NCC,MD,LMSE,NAE]=iq_measures(A,B,disp)
...
% B - array containing the stego image or its filename
...
for i=1:size(A,3)
 ...
 [MSE(i),PSNR(i),AD(i),SC(i),NCC(i),MD(i),LMSE(i),NAE(i)]=measureQ(A(:,:,i),B(:,:,i),disp,numout);
end;

function [MSE,PSNR,AD,SC,NCC,MD,LMSE,NAE]=measureQ(A,B,disp,numout)
PSNR=0;
AD=0;
SC=0;
NCC=0;
...
if (numout > 4)
 % NK
 NCC=sum(sum(A.*B))/Pk; 
 if disp~=0 fprintf('NCC (Normalised Cross-Correlation) = %f\n',NCC);end;
end;  
